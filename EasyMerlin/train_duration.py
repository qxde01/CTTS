...
def train_RNN(DATA_ROOT,model=None,opt='RMSprop',net='GRU',wp=10,n_in=467, n_out=1, n_layers=3, hidden_layer_size=512,epochs=100,batch_size=16,drop_rate=0.5,lr=0.001,seq_length=1000,pruning=False):
    ...
    with open(DATA_ROOT + 'saved/duration_%s.json' %net, 'w') as file:
        file.write(model_json)
    train_filelist=open(DATA_ROOT+'train_filelist.txt').read().split('\n')
    val_filelist = open(DATA_ROOT + 'val_filelist.txt').read().split('\n')
    ...
    train_gen = DataGenerator(train_filelist, DATA_ROOT=DATA_ROOT, ty='duration', n_in=n_in, n_out=n_out,batch_size=batch_size,seq_length=seq_length)
    val_gen = DataGenerator(val_filelist, DATA_ROOT=DATA_ROOT, ty='duration', n_in=n_in, n_out=n_out, batch_size=batch_size, seq_length=seq_length,shuffle=False)
    calllist = [ModelCheck(os.path.join(DATA_ROOT, 'saved/duration_%s_%s' % (net, hidden_layer_size)),steps=10,pruning=pruning), ]
    if pruning==True:
        print('Fine-tune pre-trained model with pruning...')
        pruning_params = {'pruning_schedule': tfmot.sparsity.keras.PolynomialDecay(initial_sparsity=0.35,
                                                                 final_sparsity=0.70,
    ...
    #model.save(DATA_ROOT + 'saved/duration_%s_%s_%06d.h5' % (net,hidden_layer_size,epochs) )
    if pruning == True:
        model = tfmot.sparsity.keras.strip_pruning(model)
    model.save(os.path.join(DATA_ROOT, 'saved/duration_%s_%s_%06d.h5' % (net, hidden_layer_size, epochs)),include_optimizer=False)
...
def get_args():
    ...
    parser.add_argument("--seq_length", '-seq', type=int, default=108, help="seq length ")
...
if __name__ == '__main__':
    ...
    if os.path.exists(DATA_ROOT+'saved')==False:
        os.mkdir(DATA_ROOT+'saved')
    ...
    if net=='DNN':
        train_RNN(DATA_ROOT,model=model,opt=opt,wp=wp, net=net, n_in=n_in, n_out=n_out,n_layers=6, hidden_layer_size=1024, drop_rate=0.35,batch_size=batch_size,epochs=epochs,lr=lr,seq_length=seq_length,pruning =pruning)
    elif net in ['GRU','LSTM','CNN','ResCNN']:
        train_RNN(DATA_ROOT,model=model,opt=opt,wp=wp, net=net, n_in=n_in, n_out=n_out, n_layers=2, hidden_layer_size=512,batch_size=batch_size,epochs=epochs, drop_rate=0.35,lr=lr,seq_length=seq_length,pruning =pruning )
    elif net in ['BLSTM','BGRU'] :
        train_RNN(DATA_ROOT,model=model,opt=opt,wp=wp, net=net, n_in=n_in, n_out=n_out, n_layers=2, hidden_layer_size=384,batch_size=batch_size,epochs=epochs, drop_rate=0.35,lr=lr,seq_length=seq_length,pruning =pruning )
    else:
        pass
