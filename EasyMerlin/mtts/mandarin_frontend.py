...
import re
import os
from jieba import posseg,add_word
...
add_word('频临',freq='10000',tag='a')

newwords= open('E:/tts/simple-merlin/pre/newwords.txt',encoding='utf8').read().split('\n')
for line in newwords:
    nw=line.split('\t')
    add_word(nw[0],freq=int(nw[1]),tag=nw[2])
...
def txt2label(txt, sfsfile=None, style='default'):
    ...
    #print(txt)
    #txt = NSWNormalizer(txt).normalize()
...
if __name__ == '__main__':
    ...
    txt = '继续#3把#1建设#3有#0中国#1特色#3社会#0主义#0事业#4推向#0前进'
    #txt = "网友“笨笨孩儿”压力不小"
