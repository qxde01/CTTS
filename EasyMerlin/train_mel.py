...
from models import DataGenerator_mel,build_Mel_model
#from sch import CosineAnnealingScheduler,WarmUpLearningRateScheduler#,LRTensorBoard,WarmUpCosineDecayScheduler
...
class ModelCheck(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        mname = '_%.4f-%05d.h5' % (current, epoch)
        #wname = '_%.4f-%06d_weights.h5' % (current, epoch)
...
def trainer(DATA_ROOT,model=None,opt='RMSprop',wp=10,n_in=467, n_out=80, n_layers=3, hidden_layer_size=512,epochs=100,batch_size=16,drop_rate=0.5,lr=0.001,seq_length=1000,pruning=False):
    if model is None:
        model ,infer= build_Mel_model(n_in=n_in, n_out=n_out, n_layers=n_layers, hidden_layer_size=hidden_layer_size,drop_rate=drop_rate)
    ...
    train_gen=DataGenerator_mel(train_filelist,DATA_ROOT  = DATA_ROOT,n_in=n_in, n_out=n_out,batch_size=batch_size,seq_length=seq_length)
    val_gen = DataGenerator_mel(val_filelist,DATA_ROOT  = DATA_ROOT, n_in=n_in, n_out=n_out,batch_size=batch_size,seq_length=seq_length, shuffle=False)
    calllist = [ ModelCheck(os.path.join(DATA_ROOT,'saved/mel_%s' % (hidden_layer_size)) ,steps=10,pruning=pruning),
    ...
    if pruning==True:
        ...
        calllist=calllist+[keras.callbacks.TensorBoard(log_dir='./logs',update_freq='epoch')]
    ...
    model.compile(loss=[keras.losses.mae,keras.losses.mae,keras.losses.mse,keras.losses.mse], optimizer=opt)

    model_json = model.to_json()
    with open(os.path.join(DATA_ROOT , 'saved/mel_%s.json' % hidden_layer_size), 'w') as file:
    ...
    model.save(os.path.join(DATA_ROOT ,  'saved/mel_%s_%05d.h5' % (hidden_layer_size,epochs)) ,include_optimizer=False)
...
if __name__ == '__main__':
    #DATA_ROOT = "E:/tts/thchs30_250_demo/"
    args = get_args()
    print(args)
    DATA_ROOT = args.DATA_ROOT  # "E:/tts/thchs30_250_demo/"
    ...
    if pretrained is not None:
        model=keras.models.load_model(os.path.join(DATA_ROOT ,'saved',pretrained))
    else:
        model=None


    trainer(DATA_ROOT,model=model,opt=opt,wp=wp, n_in=n_in, n_out=n_out,n_layers=3, hidden_layer_size=384, drop_rate=0.5,batch_size=batch_size,epochs=epochs,lr=lr,seq_length=seq_length,pruning=pruning)
