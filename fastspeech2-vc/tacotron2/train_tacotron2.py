...
class Tacotron2Trainer(Seq2SeqBasedTrainer):
    ...
    def generate_and_save_intermediate_result(self, batch):
        ...
        for idx, (mel_gt, mel_before, mel_after, alignment_history) in enumerate(
            ...
            # plot alignment
            figname = os.path.join(dirname, f"{utt_id}_alignment.png")
            fig = plt.figure(figsize=(8, 6))
            ax = fig.add_subplot(111)
            ax.set_title(f"Alignment @ {self.steps} steps")
            im = ax.imshow(alignment_history, aspect="auto", origin="lower", interpolation="none")
...
def main():
    ...
    with STRATEGY.scope():
        ...
        if len(args.pretrained) > 1:
            ...
            with open(args.outdir+ '/tacotron2.tflite', 'wb') as f:
                f.write(tflite_model)
            print('tacotron2 Model size is %f MBs.' % (len(tflite_model) / 1024 / 1024.0))
            '''
