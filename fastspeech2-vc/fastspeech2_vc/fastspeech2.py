...
from .fastspeech import TFFastSpeech, get_initializer
...
class TFFastSpeechVariantPredictor(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        for i in range(config.variant_prediction_num_conv_layers):
            ...
            self.conv_layers.append( tf.keras.layers.Dropout(config.variant_predictor_dropout_rate))
        self.conv_layers_sequence = tf.keras.Sequential(self.conv_layers)
        self.output_layer = tf.keras.layers.Dense(1)

        self.speaker_fc = tf.keras.layers.Dense(units=config.encoder_self_attention_params.hidden_size, name="speaker_fc", )
    ...
    def call(self, inputs, training=False):
        ...
        encoder_hidden_states, speaker_embeddings, attention_mask = inputs
...
class TFFastSpeech2(TFFastSpeech):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.f0_predictor = TFFastSpeechVariantPredictor( config, dtype=tf.float32, name="f0_predictor")
        self.energy_predictor = TFFastSpeechVariantPredictor( config, dtype=tf.float32, name="energy_predictor", )
        self.duration_predictor = TFFastSpeechVariantPredictor( config, dtype=tf.float32, name="duration_predictor" )
    ...
    def call(
        ...
        **kwargs,
        ...
        duration_outputs = self.duration_predictor(
            [last_encoder_hidden_states, speaker_embeddings, attention_mask])  # [batch_size, length]

        f0_outputs = self.f0_predictor(
            [last_encoder_hidden_states, speaker_embeddings, attention_mask], training=training )
        energy_outputs = self.energy_predictor(
            [last_encoder_hidden_states, speaker_embeddings, attention_mask], training=training )

        f0_embedding = self.f0_embeddings(
            tf.expand_dims(f0_gts, 2) )  # [barch_size, mel_length, feature]
        energy_embedding = self.energy_embeddings(
            tf.expand_dims(energy_gts, 2) )  # [barch_size, mel_length, feature]
    ...
    def _inference(
        self, input_ids, speaker_embeddings, speed_ratios, f0_ratios, energy_ratios, **kwargs,
    ):
        """Call logic."""
        attention_mask = tf.math.not_equal(input_ids, 0)
        ...
        # rather than just use last_hidden_states of encoder for energy_predictor.
        duration_outputs = self.duration_predictor(
            [last_encoder_hidden_states, speaker_embeddings, attention_mask]
        )  # [batch_size, length]
        ...
        f0_outputs = self.f0_predictor(
            [last_encoder_hidden_states, speaker_embeddings, attention_mask], training=False
        )
