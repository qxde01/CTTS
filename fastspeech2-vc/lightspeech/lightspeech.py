...
# https://github.com/nmfisher/TensorFlowTTS

import tensorflow as tf
import numpy as np
from tensorflow_tts.models.fastspeech import TFFastSpeechAttention, TFFastSpeechOutput, TFFastSpeechEmbeddings, \
    TFFastSpeechLengthRegulator, ACT2FN, get_initializer, TFEmbedding, TFTacotronPostnet
...
class TFLightSpeechLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, index, **kwargs):
        ...
        self.intermediate = TFLightSpeechIntermediate(config, index, name="intermediate")
...
class TFLightSpeech(tf.keras.Model):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.encoder = TFLightSpeechEncoder(config.encoder_self_attention_params, name="encoder" )
        self.length_regulator = TFFastSpeechLengthRegulator(config,enable_tflite_convertible=self.enable_tflite_convertible,
                                                            name="length_regulator", )
        self.decoder = TFLightSpeechDecoder(config.decoder_self_attention_params,
            is_compatible_encoder=config.encoder_self_attention_params.hidden_size == config.decoder_self_attention_params.hidden_size,name="decoder", )
        ...
        self.f0_predictor = TFLightSpeechVariantPredictor(config, dtype=tf.float32, name="f0_predictor")
        self.energy_predictor = TFLightSpeechVariantPredictor(config, dtype=tf.float32, name="energy_predictor")
        self.duration_predictor = TFLightSpeechVariantPredictor(config, dtype=tf.float32, name="duration_predictor")

        # define f0_embeddings and energy_embeddings
        self.f0_embeddings = tf.keras.layers.Conv1D(filters=config.encoder_self_attention_params.hidden_size,kernel_size=9,padding="same", name="f0_embeddings", )
        self.f0_dropout = tf.keras.layers.Dropout(0.5)

        self.energy_embeddings = tf.keras.layers.Conv1D(filters=config.encoder_self_attention_params.hidden_size, kernel_size=9,padding="same",name="energy_embeddings", )
