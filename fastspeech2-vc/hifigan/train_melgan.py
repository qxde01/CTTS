...
class MelganTrainer(GanBasedTrainer):
    ...
    def compute_per_example_generator_losses(self, batch, outputs):
        ...
        for i in range(len(p_hat)):
            adv_loss += calculate_3d_loss( tf.ones_like(p_hat[i][-1]), p_hat[i][-1], loss_fn=self.mse_loss )
        adv_loss /= i + 1
    ...
    def compute_per_example_discriminator_losses(self, batch, gen_outputs):
        ...
        for i in range(len(p)):
            real_loss += calculate_3d_loss( tf.ones_like(p[i][-1]), p[i][-1], loss_fn=self.mse_loss )
            fake_loss += calculate_3d_loss( tf.zeros_like(p_hat[i][-1]), p_hat[i][-1], loss_fn=self.mse_loss )
        real_loss /= i + 1
        fake_loss /= i + 1
