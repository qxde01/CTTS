...
try:
    from hifigan.audio_mel_dataset import AudioMelDataset
    from hifigan.train_melgan import collater
    from hifigan.train_melgan_stft import MultiSTFTMelganTrainer
except:
    from audio_mel_dataset import AudioMelDataset
    from train_melgan import collater
    from train_melgan_stft import MultiSTFTMelganTrainer
...
def main():
    ...
    parser = argparse.ArgumentParser( description="Train Hifigan (See detail in examples/hifigan/train_hifigan.py)" )
    parser.add_argument( "--train-dir",default=None,type=str, help="directory including training data. ",)
    parser.add_argument("--dev-dir", default=None, type=str, help="directory including development data. ",  )
    parser.add_argument( "--use-norm", default=1, type=int, help="use norm mels for training or raw." )
    ...
    parser.add_argument( "--generator_mixed_precision",default=0, type=int,  help="using mixed precision for generator or not.", )
    parser.add_argument( "--discriminator_mixed_precision",default=0,type=int, help="using mixed precision for discriminator or not.",  )
    parser.add_argument("--pretrained", default="", type=str,  nargs="?",  help="path of .h5 melgan generator to load weights from",  )
    ...
    # set logger
    if args.verbose > 1:
        logging.basicConfig(level=logging.DEBUG,stream=sys.stdout,format="%(asctime)s (%(module)s:%(lineno)d) %(levelname)s: %(message)s", )
    elif args.verbose > 0:
        logging.basicConfig(level=logging.INFO,stream=sys.stdout,format="%(asctime)s (%(module)s:%(lineno)d) %(levelname)s: %(message)s",  )
    else:
    ...
    if config["remove_short_samples"]:
        mel_length_threshold = config["batch_max_steps"] // config["hop_size" ] + 2 * config["hifigan_generator_params"].get("aux_context_window", 0)
    else:
    ...
    with STRATEGY.scope():
        ...
        generator = TFHifiGANGenerator(
            HifiGANGeneratorConfig(**config["hifigan_generator_params"]),  name="hifigan_generator",  )
       ...
       '''
        ...
        gen_optimizer = tf.keras.optimizers.Adam(
            learning_rate=learning_rate_fn,beta_1=0.8,beta_2=0.99,
