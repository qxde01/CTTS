...
from RepVGG import SpeakerRegVGG
...
def trainer(train_dataset, valid_dataset, config):
    ...
    for step, (x_batch_train, y_batch_train) in tqdm(enumerate(train_dataset), ncols=80, total=config["train_total_steps"],
        ...
        if step % config["log_step_interval"] == 0 and step > config["init_step"]:
        ...
        valid_loss=0.0
        if (step % config["save_step_interval"] == 0 and step > config["init_step"]) or (step==config["train_total_steps"]-1):
            ...
            if config['activation'] == 'amsoftmax':
                model_amsoftmax.save_weights(filepath='saved/%s-amsoftmax-w-%s-%d-%.4f.h5' % (net, config["classes"], step, float(val_acc)))
            else:
                model_softmax.save_weights(filepath='saved/%s-softmax-w-%s-%d-%.4f.h5' % (net, config["classes"], step, float(val_acc)))
...
def parse_fun(item, label):
    ...
    return tf.expand_dims(a_mel,axis=-1), label
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_data", "-train", default='data/train_100_4531.csv', type=str, help="train data", )
    parser.add_argument("--valid_data", "-valid", default='data/valid_100_4531.csv', type=str, help="valid data", )
    parser.add_argument("--net", "-net", default='RepVGG', type=str, help="ResNetV1,ResNetV2", )
    ...
    with tf.device("/cpu:0"):
        ...
        train_dataset = train_dataset.padded_batch(batch_size, padded_shapes=([None, None,1], []))
        ...
        valid_dataset = valid_dataset.padded_batch(batch_size, padded_shapes=([None, None,1], []))

    #if net == "RegVGG":
    model_softmax, model_amsoftmax, speark_emb_model=SpeakerRegVGG(input_shape=(None,None,80,1),classes=config["classes"])
